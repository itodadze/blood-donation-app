...
def insert_user_by_location(
    icon: UserIcon, blood: BloodType, loc_latitude: float, loc_longitude: float
) -> User:
...
def default_broadcast_search_request(
    user: User, blood: BloodType, loc_latitude: float, loc_longitude: float
...
class DonorPriorityRankerTestCase(TestCase):
    ...
    def test_donor_priority_ranking(self) -> None:
        ranker = DonorPriorityRanker(
            default_broadcast_search_request(self.user, self.a_minus, 14.0, 8.0)
        )
