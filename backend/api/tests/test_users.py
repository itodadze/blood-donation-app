...
class UsersTestCase(TestCase):
    def setUp(self) -> None:
        ...
        self.user: User = insert_default_user(self.icon, self.o_plus, "Test", "Subject1")
    ...
    def test_patch_user_partially(self) -> None:
        request = MagicMock(spec=Request)
        request.data = UserUpdateRequest(id=self.user.pk, icon_id=self.icon.pk).as_dictionary()
